    ...
    <div class="flex-1 flex flex-col w-full md:ml-72 overflow-hidden transition-all duration-300">
        ...
        <main class="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 bg-slate-50">
                        ...
                        <div class="px-6 py-6 space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <p id="detailNama" class="text-lg font-bold text-slate-900">-</p>
                                </div>
                                <div>
                                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">Harga Satuan</label>
                                    <p id="detailHarga" class="text-lg font-bold text-blue-600">-</p>
                                </div>
                            </div>
                        
                            <div>
                                <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">Deskripsi Spesifikasi</label>
                                <p id="detailDeskripsi" class="text-sm text-slate-600 bg-slate-50 p-3 rounded-lg border border-slate-100 leading-relaxed max-h-24 overflow-y-auto">-</p>
                            </div>
            ...
            <div id="modalExport" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4">
                <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-md transition-opacity" onclick="hideModal('modalExport')"></div>

                <div class="relative bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 flex flex-col z-10">
                    ...
                    <form id="formExport" action="{{ route('brgmanajer.export') }}" method="GET" class="p-8">
                        ...
                        <div id="categoryGrid" class="grid grid-cols-2 md:grid-cols-3 gap-3 max-h-48 overflow-y-auto pr-2">
            ...
            <div id="modalExportConfirm" class="fixed inset-0 z-50 hidden overflow-y-auto">
                <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center w-full">
                    <div id="closeExportConfirmOverlay" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity"></div>
                
                    <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>

                    <div class="relative inline-block align-bottom bg-white rounded-2xl text-center shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-slate-200 overflow-hidden z-10">
    ...
    <script>
        ...
        document.getElementById('btnConfirmExport').addEventListener('click', function() {
            ...
            try {
                ...
                form.method = 'GET';
                form.action = '{{ route('brgmanajer.export') }}';
                form.target = '_blank';
